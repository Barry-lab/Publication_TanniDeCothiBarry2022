...
class _Half_ViolinPlotter(_CategoricalPlotter):
    ...
    def __init__(self, x, y, hue, data, order, hue_order,
                 ...
                 color, palette, saturation, offset, bound_kda_by_95_percentiles=False):

        self.establish_variables(x, y, hue, data, orient, order, hue_order)
        self.establish_colors(color, palette, saturation)
        self.estimate_densities(bw, cut, scale, scale_hue, gridsize,
                                bound_kda_by_95_percentiles=bound_kda_by_95_percentiles)
    ...
    def estimate_densities(self, bw, cut, scale, scale_hue, gridsize, bound_kda_by_95_percentiles=False):
    ...
    def kde_support(self, x, bw, cut, gridsize, bound_kda_by_95_percentiles=False):
        """Define a grid of support for the violin."""
        xmin = np.quantile(x, 0.05) if bound_kda_by_95_percentiles else x.min()
        xmax = np.quantile(x, 0.95) if bound_kda_by_95_percentiles else x.max()
        support_min = xmin - bw * cut
        support_max = xmax + bw * cut
...
def half_violinplot(x=None, y=None, hue=None, data=None, order=None, hue_order=None,
               ...
               ax=None, offset=.15, bound_kda_by_95_percentiles=False, **kwargs):
