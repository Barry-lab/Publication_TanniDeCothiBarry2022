...
def stat_test(box_data1, box_data2, test, bonferroni_m=1, **stats_params):
    test_short_name = ''
    pval = None
    formatted_output = None
    if test == 'Levene':
        stat, pval = stats.levene(box_data1, box_data2, **stats_params)
        pval = min(pval * bonferroni_m, 1)
...
def add_stat_annotation(ax, plot='boxplot',
                        ...
                        line_offset=None, line_height=0.02, text_offset=1, text_va='bottom',
                        color='0.2', linewidth=1.5,
                        fontsize='medium', cut_kde_to_95_percentile=False,
                        apply_bonferroni_correction=False, verbose=1):
    """
    ...
    if cut_kde_to_95_percentile:
        ymax = [np.quantile(get_box_data(box_plotter, box_names[i]), 0.95)
    ...
    for box_struct1, box_struct2 in box_struct_pairs:
        ...
        if perform_stat_test:
            bonferroni_m = len(box_struct_pairs) if apply_bonferroni_correction else 1
            pval, formatted_output, test_short_name = stat_test(box_data1, box_data2, test,
                                                                bonferroni_m=bonferroni_m, **stats_params)
        else:
        ...
        if text is not None:
            ann = ax.annotate(
                ...
                xycoords='data', ha='center', va=text_va,
